...
async function fetchNews() {
  ...
  try {
    const response = await fetch(
      "https://f6da7095-a6f6-4d17-92c3-5fe84333d397-00-i370d3pysjru.pike.replit.dev/news.php"
    );
...
function applyFilters() {
  ...
  setTimeout(() => {
    const checkedFilters = document.querySelectorAll(
      ".filter-menu input:checked"
    );
    activeFilters = new Set(
      Array.from(checkedFilters).map((input) => input.value)
    );

    if (activeFilters.size > 0) {
      newsData = originalNewsData.filter((news) =>
        activeFilters.has(news["news-tag"])
      );
    } else {
...
function applySorting(sortType, showLoadingState = true) {
  ...
  setTimeout(() => {
    switch (sortType) {
      case "date-new":
        newsData.sort(
          (a, b) => new Date(b["news-date"]) - new Date(a["news-date"])
        );
        break;
      case "date-old":
        newsData.sort(
          (a, b) => new Date(a["news-date"]) - new Date(b["news-date"])
        );
        break;
      case "alpha-asc":
        ...
        <article class="item-card">
            <div class="item-content">
                <div class="item-meta">
                    ...
                    <span class="item-date">${formatDate(
                      news["news-date"]
                    )}</span>
                </div>
                <h2 class="item-title">${news["news-title"]}</h2>
                <p class="item-desc">${news["news-short-desc"]}</p>
                <a href="view-news.html?id=${news["news-id"]}" class="module-link">Read More <i class="fas fa-arrow-right"></i></a>
            </div>
        </article>
    `
    )
...
searchInput.addEventListener("input", (e) => {
  ...
  searchTimeout = setTimeout(() => {
    ...
    if (searchTerm) {
      newsData = originalNewsData.filter(
        (news) =>
